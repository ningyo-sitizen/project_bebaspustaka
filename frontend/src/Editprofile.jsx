...
const EditProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put("http://localhost:5000/api/profile/edit", {
        name: formData.name,
        username: formData.username,
        // Kirim password baru hanya jika ada isinya
        newPassword: formData.newPassword || undefined, 
      });

      // 3. Penanganan Respons Sukses
      if (response.status === 200) {
        alert("Profil berhasil diperbarui!");
        // Update state lokal dan reset password fields
        setFormData(prev => ({
            ...prev,
        ...
        navigate("/profile"); // Pindah ke halaman profile setelah sukses
      }
    } catch (error) {
      // 4. Penanganan Error
      const errorMessage = error.response?.data?.message || "Terjadi kesalahan koneksi atau server.";
